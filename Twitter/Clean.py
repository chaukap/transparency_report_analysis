...
twitter["half"] = [2 if "07" in t else 1 for t in twitter["Time period start"]]
...
twitter["time"] = twitter.apply(lambda y: ("Jan-Jun" if y.half == 1 else "Jul-Dec") + " " + y.year, axis=1)
...
twitter.to_csv("../Clean_Data/Twitter.csv", index=False)